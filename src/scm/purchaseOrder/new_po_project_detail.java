...
public class new_po_project_detail extends javax.swing.JFrame {

    static DBControl db = new DBControl();

    public new_po_project_detail() {
        initComponents();
        load();
    }
    ...
    private void initComponents() {
        ...
        setSize(new java.awt.Dimension(668, 498));
    ...
    private void bSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bSaveActionPerformed
        if (txtProjectDetails.getText().isEmpty()) {
            ...
            try {
                ResultSet rss = DBControl.getResultFromLocalDB("SELECT project_detail FROM new_po_project_detail WHERE project_detail='" + txtProjectDetails.getText() + "'");
                if (rss.next()) {
                    update();
                } else {
                    try {
                        db.setResult("INSERT INTO new_po_project_detail VALUES('" + txtProjectDetails.getText() + "')");
                        load();
                        loadCombo();
                    } catch (Exception ex) {
                        Logger.getLogger(new_po_project_detail.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            } catch (Exception ex) {
                Logger.getLogger(new_po_project_detail.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void bDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bDeleteActionPerformed
        if (txtProjectDetails.getText().isEmpty()) {
            ...
            try {
                ...
                if (i == 0) {
                    db.setResult("DELETE From new_po_project_detail  WHERE project_detail='" + txtProjectDetails.getText() + "'");
    ...
    private void load() {
        try {
            ResultSet rs = DBControl.getResultFromLocalDB("SELECT * FROM new_po_project_detail");
            DefaultTableModel dtm = (DefaultTableModel) jTable1.getModel();
            dtm.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
    ...
    private void loadCombo() {
        try {
            ResultSet rs1 = DBControl.getResultFromLocalDB("SELECT project_detail FROM new_po_project_detail");
            Vector v2 = new Vector();
            while (rs1.next()) {
                v2.add(rs1.getString("project_detail"));
            }
            new_po.comboProject.setModel(new DefaultComboBoxModel(v2));
        } catch (Exception ex) {
            Logger.getLogger(new_po_project_detail.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void update() {
        try {
            db.setResult("UPDATE new_po_project_detail SET project_detail = '" + txtProjectDetails.getText() + "' WHERE project_detail = '" + txtProjectDetails.getText() + "'");
            load();
            loadCombo();
        } catch (Exception ex) {
