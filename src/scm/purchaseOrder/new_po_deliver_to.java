...
public class new_po_deliver_to extends javax.swing.JFrame {

    static DBControl db = new DBControl();

    public new_po_deliver_to() {
        initComponents();
        load();
    }
    ...
    private void initComponents() {
        ...
        jScrollPane1.setBounds(20, 170, 450, 210);
        ...
        txtName.setBounds(80, 70, 390, 23);
        ...
        jToolBar1.setBounds(0, 0, 480, 70);
        ...
        jScrollPane2.setBounds(80, 100, 390, 60);

        setSize(new java.awt.Dimension(502, 464));
        setLocationRelativeTo(null);
    }// </editor-fold>//GEN-END:initComponents

    private void txtNameKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtNameKeyReleased
        try {
            ResultSet rs = DBControl.getResultFromLocalDB("SELECT * FROM new_po_deliver_to WHERE name LIKE '" + txtName.getText() + "%'");
    ...
    private void bSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bSaveActionPerformed
        if (txtName.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Enter a new Delivery Location to save", "Error", JOptionPane.ERROR_MESSAGE);
        } else {
            try {
                ResultSet rss = DBControl.getResultFromLocalDB("SELECT name FROM new_po_deliver_to WHERE name='" + txtName.getText() + "'");
                if (rss.next()) {
                    update();
                } else {
                    try {
                        db.setResult("INSERT INTO new_po_deliver_to VALUES('" + txtName.getText() + "','" + txtAddress.getText() + "')");
                        load();
                        loadCombo();
                    } catch (Exception ex) {
                        Logger.getLogger(new_po_deliver_to.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void bDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bDeleteActionPerformed
        if (txtName.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Select a Delivery Location to delete !", "Error", JOptionPane.ERROR_MESSAGE);
        } else {
            try {
                int i = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (i == 0) {
                    db.setResult("DELETE From new_po_deliver_to  WHERE name='" + txtName.getText() + "'");
                    txtName.setText("");
                    txtAddress.setText("");
                    loadCombo();
                } else if (i == 1) {
    ...
    private void load() {
        try {
            ResultSet rs = DBControl.getResultFromLocalDB("SELECT * FROM new_po_deliver_to");
            DefaultTableModel dtm = (DefaultTableModel) jTable1.getModel();
            dtm.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
    ...
    private void loadCombo() {
        try {
            ResultSet rs1 = DBControl.getResultFromLocalDB("SELECT address FROM new_po_deliver_to");
            Vector v2 = new Vector();
            while (rs1.next()) {
                v2.add(rs1.getString("address"));
            }
            new_po.comboDeliverTo.setModel(new DefaultComboBoxModel(v2));
        } catch (Exception ex) {
            Logger.getLogger(new_po_deliver_to.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void update() {
        try {
            db.setResult("UPDATE new_po_deliver_to SET address = '" + txtAddress.getText() + "' WHERE name = '" + txtName.getText() + "'");
            load();
            loadCombo();
        } catch (Exception ex) {
            Logger.getLogger(new_po_deliver_to.class.getName()).log(Level.SEVERE, null, ex);
